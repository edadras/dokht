...
/*
 * برخوردگرهای بدن — همان‌هایی که نماگر می‌سازد، نه رونوشتشان.
 *
 * این‌جا یک نسخهٔ دست‌نویس بود و همان اشتباهِ جدولِ بدن دوباره تکرار شده بود:
 * سنجه لباس را به بدنی می‌کوباند که مرورگر نمی‌سازد. نتیجه‌اش این شد که سنجه
 * می‌گفت «✓ روی مانکن» و در مرورگر همان کت با «قطعه‌ای از لباس بالای سر رفت»
 * رد می‌شد و به نمای چرخشیِ قدیمی برمی‌گشت. سه مدل از پنج مدل این‌طور بودند و
 * هیچ عددی نگفت.
 *
 * garment-solid.js با three کار می‌کند ولی آن را *تنبل* بار می‌کند، پس همین
 * یک تابع در نود هم قابل صدا زدن است.
 */
export { bodyColliders } from '../../../resources/js/components/garment-solid.js';
