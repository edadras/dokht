 ...
 *
 * و تا *پیش از* شبیه‌سازی زده می‌شود، نه به امیدِ آن.
 *
 * قید فقط دو سوی خط را به هم نزدیک می‌کند و نمی‌گوید به کدام سو تا شود. پارچه
 * راهِ ارزان‌تری پیدا می‌کرد: کلِ نوار را در یک صفحه می‌خواباند. روی پیراهن،
 * آنچه دیده می‌شد یقه‌ای ایستاده با رویهٔ برگشته نبود؛ نوارِ مچاله‌ای بود پهن‌شده
 * روی شانه‌ها — و همان، ناحیهٔ سرشانه را در هر مدلی خراب نشان می‌داد. خیاط هم
 * یقه را اول اتو می‌کند بعد می‌دوزد.
 *
 * سه چیز اینجا قفل می‌شود:
 *   ۱. یقه از خودِ buildDrape تاشده بیرون می‌آید (نصفِ بلندیِ تختش).
 *   ۲. تا در شبیه‌سازی باز نمی‌شود.
 *   ۳. weldSeams تا را خطا نمی‌شمارد. نسخهٔ پیش از این، تای بسته را «درزِ
...
test('یقه روی خط خواب تا می‌شود و جوش تا را باز نمی‌کند', () => {
    ...
    assert.equal(drape.stats.folded, 1, 'یقه باید همان‌جا در ساخت اتو شود');
    assert.ok(cut.tall < 5, `یقهٔ چیده ${cut.tall.toFixed(1)} سانتی‌متر بلند است؛ تا نخورده`);
    assert.ok(
        seeded.now < seeded.flat * 0.4,
        `تای اولیه بسته نیست: آینه‌ها ${(seeded.now * 100).toFixed(1)} سانتی‌متر فاصله دارند و روی الگو ${(seeded.flat * 100).toFixed(1)}`,
    );
    ...
    assert.ok(
        ...
        `تا باز شد: آینه‌ها روی پارچه ${(shut.now * 100).toFixed(1)} سانتی‌متر فاصله دارند و روی الگو ${(shut.flat * 100).toFixed(1)}`,
    );
    assert.ok(on.tall < cut.tall + 2, `بلندی یقه از ${cut.tall.toFixed(1)} به ${on.tall.toFixed(1)} رفت؛ تا باز شده است`);
