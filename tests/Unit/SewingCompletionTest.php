...
class SewingCompletionTest extends TestCase
{
     ...
     *
     * این‌ها را «درست» نکرده‌ایم چون درزشان را هندسه نمی‌تواند حدس بزند: پیشبندِ
     * اورال روی تنه می‌نشیند نه در درز، و لایه‌های آستینِ لایه‌ای اصلاً به هم
     * دوخته نمی‌شوند.
     *
     * ترکِ دامنِ ترک‌دار و گودت زمانی همین‌جا بود و حالا نیست: راهش — همان
     * برچسب‌گذاری در خودِ ژنراتور — زده شد. هر ترک می‌گوید همسایه‌اش کدام است
     * (meta.seam_neighbours) و دوخت‌یاب فقط می‌خواندش. تا پیش از آن، «جلو به
     * پشت» هم‌طول‌ترین جفت بود و همان اول برداشته می‌شد، پس چهار ترکِ پهلو
     * بی‌شریک می‌ماندند و دامن روی مانکن تکه‌تکه می‌شد.
     */
    protected const ALLOW_LOOSE = [
        'dress_mermaid', 'overall', 'sleeve_layered',
    ];
     ...
     * «neck» نداشت و سازندهٔ رابطه‌ها — که خط یقه را فقط از همان برچسب
     * می‌شناسد — هیچ‌وقت به گردن نمی‌دوختش. روی قپائو از ۱۰۹ سانتی‌متر محیط
     * تنها ۴٫۵ دوخته بود، و آن ۴٫۵ به سجاف رفته بود.
     *
     ...
     * دوخته می‌شد و از بازو کنده می‌شد.
     */
}
