...
class CatalogAuditTest extends TestCase
{
    ...
    protected const ALLOW_LIST = [
        ...
        'side_seam:skirt_wrap_*' => 'همان دامن پاکتی با قدهای دیگر؛ جلوی روی‌هم‌آمده عمداً بلندتر از پشت است.',
        'side_seam:skirt_tulip_*' => 'همان دامن لاله با قدهای دیگر؛ لبهٔ روی‌هم‌آمدن گلبرگ‌ها با درز پهلوی پشت جفت نمی‌شود.',
        ...
        'hip_girth:trad_dishdasha' => 'لباسِ محلیِ رها از سرشانه می‌ریزد و روی باسن نمی‌نشیند؛ آزادیِ چهل سانتی‌متریِ باسن شناسنامهٔ همین سایه است، نه خطای اندازه.',
        'hip_girth:trad_ferace' => 'لباسِ محلیِ رها از سرشانه می‌ریزد و روی باسن نمی‌نشیند؛ آزادیِ چهل سانتی‌متریِ باسن شناسنامهٔ همین سایه است، نه خطای اندازه.',
        'hip_girth:trad_dashiki' => 'لباسِ محلیِ رها از سرشانه می‌ریزد و روی باسن نمی‌نشیند؛ آزادیِ چهل سانتی‌متریِ باسن شناسنامهٔ همین سایه است، نه خطای اندازه.',
        'hip_girth:trad_turkmen' => 'لباسِ محلیِ رها از سرشانه می‌ریزد و روی باسن نمی‌نشیند؛ آزادیِ چهل سانتی‌متریِ باسن شناسنامهٔ همین سایه است، نه خطای اندازه.',
        'hip_girth:trad_bandari' => 'لباسِ محلیِ رها از سرشانه می‌ریزد و روی باسن نمی‌نشیند؛ آزادیِ چهل سانتی‌متریِ باسن شناسنامهٔ همین سایه است، نه خطای اندازه.',
        'hip_girth:trad_baluchi' => 'لباسِ محلیِ رها از سرشانه می‌ریزد و روی باسن نمی‌نشیند؛ آزادیِ چهل سانتی‌متریِ باسن شناسنامهٔ همین سایه است، نه خطای اندازه.',
        'hip_girth:trad_jalabiya' => 'لباسِ محلیِ رها از سرشانه می‌ریزد و روی باسن نمی‌نشیند؛ آزادیِ چهل سانتی‌متریِ باسن شناسنامهٔ همین سایه است، نه خطای اندازه.',
        'hip_girth:trad_yukata' => 'لباسِ محلیِ رها از سرشانه می‌ریزد و روی باسن نمی‌نشیند؛ آزادیِ چهل سانتی‌متریِ باسن شناسنامهٔ همین سایه است، نه خطای اندازه.',
        'hip_girth:trad_huipil' => 'لباسِ محلیِ رها از سرشانه می‌ریزد و روی باسن نمی‌نشیند؛ آزادیِ چهل سانتی‌متریِ باسن شناسنامهٔ همین سایه است، نه خطای اندازه.',
        'hip_girth:trad_kurdish_dress' => 'لباسِ محلیِ رها از سرشانه می‌ریزد و روی باسن نمی‌نشیند؛ آزادیِ چهل سانتی‌متریِ باسن شناسنامهٔ همین سایه است، نه خطای اندازه.',
    ...
    protected function checkOutline(string $key, string $size, array $piece): array
    {
        ...
        foreach ([$piece] as $piece) {
            ...
            if ($area > static::MAX_AREA && ! $this->allowed('max_area', $key)) {
    ...
    protected function checkGirths(string $key, string $size, array $pieces): array
    {
        ...
        foreach ([$size] as $size) {
            ...
            /*
             * پیش‌تر این‌جا فقط سینه مرجع بود، چون روی جدولِ سایز سینه از کمر
             * بزرگ‌تر است و باسن هم‌اندازه یا بزرگ‌ترش. ولی بدنِ آدم‌ها همیشه
             * این‌طور نیست: اندام سیبی کمرش از سینه و باسنش بزرگ‌تر است. آن‌وقت
             * لباسی که باید از روی کمر رد شود، ناچار روی باسن گشاد می‌افتد — و
             * این شکل بدن است، نه ایراد درفت.
             *
             * پس مرجع، بزرگ‌ترین دورِ همان بدن است، هر کدام که باشد.
             */
            $widest = max($body['bust'], $body['waist'], $body['hip']);

            $drop = [
                'bust' => max(0.0, $widest - $body['bust']),
                'waist' => max(0.0, $widest - $body['waist']),
                'hip' => max(0.0, $widest - $body['hip']),
            ];
    ...
    public function test_building_the_whole_catalogue_stays_fast(): void
    {
        ...
        /*
         * سقفِ زمان به‌ازای *هر مدل* است، نه جمعِ کل.
         *
         * سقفِ مطلق با هر مدلِ تازه یک قدم به شکستن نزدیک می‌شد و آخرش هم شکست —
         * بی آنکه چیزی کند شده باشد: ۲۵۶۸ مدل در ۱۲ ثانیه یعنی هر مدل ۴٫۷
         * میلی‌ثانیه، که از ۵۴۰ مدلِ پیشین *سریع‌تر* است. آنچه باید بپاید سرعتِ
         * ساختِ یک الگوست، و آن با بزرگ شدنِ کاتالوگ عوض نمی‌شود.
         */
        $perModel = $total / max(1, count($keys));

        $this->assertLessThan(
            8.0,
            $perModel,
            sprintf('ساخت هر مدل به‌طور میانگین %.1f میلی‌ثانیه طول کشید (%d مدل).', $perModel, count($keys)),
        );
        /*
    ...
    protected function waistGirth(array $pieces, array $parts): float
    {
        ...
        foreach ($this->partsLike($pieces, $parts) as $piece) {
            ...
            // ساسون کمر روی بالاتنهٔ جذب هم شماره لبه ندارد (پایش به نوک می‌رود،
            // نه روی لبه)، ولی دهانه‌اش از همان لبهٔ کمر خورده می‌شود. بدون این،
            // کمر هر بالاتنهٔ ساسون‌دار به‌اندازهٔ ساسون‌هایش بزرگ‌تر شمرده می‌شود.
