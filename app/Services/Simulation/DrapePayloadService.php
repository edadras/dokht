...
class DrapePayloadService
{
     ...
     */
    protected const SPLIT_TAGS = ['neck', 'armhole', 'shoulder', 'waist', 'crotch'];
     ...
     */
    protected const SPLIT_STEPS = ['waist' => 2.0, 'crotch' => 2.0];
    ...
    public function payload(Pattern $pattern): array
    {
        ...
        foreach ($models as $model) {
            /*
             * آستر داخل لباس است و نباید به‌عنوان پوستهٔ مستقل روی مانکن شبیه‌سازی
             * شود. حضور هم‌زمان رو و آسترِ هم‌شکل، دو سطح را در یک مکان می‌گذاشت؛
             * حل‌گر برخورد آن‌ها را از هم هل می‌داد و لباس بادکرده یا پیچیده دیده
             * می‌شد. جنس و وزن آستر در مشخصات پارچه لحاظ می‌شود، اما هندسهٔ قابل
             * مشاهده فقط از لایهٔ بیرونی ساخته می‌شود.
             */
            if ((string) ($model->layer ?? 'outer') === 'lining') {
                $notes[] = "قطعه «{$model->code}» آستر داخلی است و در پوستهٔ نمای سه‌بعدی جداگانه نمایش داده نمی‌شود.";
    ...
    protected function matchRole(string $haystack): ?string
    {
        ...
        /*
         * «bandeau» یک مدل بالاتنه است، نه واژهٔ انگلیسی band.
         *
         * جست‌وجوی زیررشته‌ایِ `band` باعث می‌شد تمام قطعات بیکینی بندو
         * detail تشخیص داده شوند. در نتیجه قطعات شورت به‌علت نداشتن رابطهٔ
         * صریح حذف می‌شدند و تنها یک نوار مورب میان سینه و فاق باقی می‌ماند.
         * نقش‌های اعلام‌شدهٔ خود ژنراتور را پیش از واژه‌های عمومی می‌خوانیم.
         */
        if (str_contains($haystack, 'swim_bottom')) {
            return 'skirt';
        }

        if (str_contains($haystack, 'bodice')) {
            return 'torso';
        }
    ...
    protected function side(PatternPiece $piece, bool $mirrored, int $quantity, int $index = 0): ?string
    {
        ...
        /*
         * تاپ بندو دو مستطیل هم‌شکل دارد: یکی جلو و یکی پشت. چون این دو نه
         * mirror هستند و نه meta.side داشتند، هر دو دقیقاً روی جلوی تن قرار
         * می‌گرفتند و dedupe دومی را حذف می‌کرد. girth_factor=2 قرارداد خود
         * ژنراتور برای همین دو نیمهٔ یک دور کامل است.
         */
        if ($quantity === 2
            && ! $piece->mirror
            && ($piece->meta['part'] ?? null) === 'front_bodice'
            && (int) ($piece->meta['girth_factor'] ?? 0) === 2) {
