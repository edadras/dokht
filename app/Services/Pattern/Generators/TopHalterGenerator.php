...
class TopHalterGenerator extends TopBaseGenerator
{
    ...
    public function generate(array $measurements, array $ease, array $params): array
    {
        ...
        // جلو بریده نمی‌شود (حلقه دارد)، پس خط بالای پشت باید دقیقاً روی همان
        // نقطهٔ زیر بغلِ جلو به درز پهلو برسد؛ وگرنه دو درز پهلو هم‌اندازه نیستند
        $backTop = (float) ($back['meta']['bust_y'] ?? 20) - $backDrop;
        $back = $this->cutTop($back, [
            'center' => $backTop,
            'side' => (float) ($front['meta']['bust_y'] ?? $backTop),
            'shape' => 'straight',
        ]);
