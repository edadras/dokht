...
abstract class BaseGenerator implements PatternGenerator
{
    ...
    protected function markSkirtZip(array $pieces, string $where): array
    {
        ...
        /*
         * زیپِ مرکزِ پشت فقط روی پنلی معنا دارد که تا حالا روی تای پارچه بوده.
         *
         * باز کردنِ تا یعنی همان نیم‌پنل دو بار قرینه بریده می‌شود و میانشان
         * درزی می‌ماند که زیپ در آن می‌نشیند. ولی اگر پنل از اول *کاملِ* پشت
         * باشد (دامنِ نامتقارن این‌طور است)، دو کردنِ شمارهٔ برش یعنی دستور به
         * بریدنِ دو پشتِ کامل — دو برابرِ پارچه و لباسی که دوخته نمی‌شود. آن
         * پنل اصلاً درزِ مرکزِ پشت ندارد که زیپ در آن بنشیند.
         *
         * پس روی پنلِ کامل، زیپ به درزِ پهلو می‌رود؛ همان‌جایی که واقعاً هست.
         */
        if ($where === 'back' && empty($piece['on_fold'])) {
            $where = 'side';
        }
