  ...
  "pieces": [
    {
      ...
      "placement": {
        ...
        "y_end": 0.74,              // اختیاری: ارتفاع همان لبه در سرِ دیگرِ قطعه.
                                    // اگر بیاید، قطعه مورب می‌نشیند نه افقی —
                                    // نوارِ لبهٔ راپ که از کمر تا یقه می‌رود
        "radius_hint": "bust",      // کدام تراز بدن شعاعِ چیدن اولیه را می‌دهد
        "laps": false,              // این پنل از خط مرکز رد می‌شود (جلوی راپ)
...
- چیدن اولیه: هر قطعه روی استوانه‌ای با شعاعِ `radius_hint` و بازهٔ `u0..u1`
  پیچیده می‌شود، لبهٔ بالا روی `y_top` — و اگر `y_end` آمده باشد، ارتفاع در طولِ
  قطعه از `y_top` تا `y_end` میان‌یابی می‌شود (نوارِ مورب). این فقط نقطهٔ شروع
  است؛ درزها بقیه را جمع می‌کنند.
...
## خروجیِ ماشینی برای موتورهای بیرونی — `sew3d`

همین بسته از بیرون هم گرفتنی است، برای هر شبیه‌سازِ دیگری که بخواهد لباسِ
Dokht را بدوزد — XPBD روی GPU، حل‌کنندهٔ کیفیت (FEM/IPC)، یا بهینه‌سازِ الگو.
معماریِ دو-حالته («بی‌درنگ» در مرورگر، «کیفیت» روی سرورِ GPU) دقیقاً از همین
در وارد می‌شود: ورودیِ هر دو حالت یکی است.

```
GET /patterns/{pattern}/export/sew3d
php artisan drape:bench --model=…      # همین بسته، روی دیسک، برای سنجه
```
...
{
  "format": "dokht-sew3d",
  "format_version": 1,               // با هر تغییرِ ناسازگار بالا می‌رود
  "exported_at": "…",
  "pattern": { "id": 12, "name": "…", "generator": "suit_jacket", "version": 3 },
  "drape": { … },                    // همان بستهٔ بالا، بی هیچ تفاوتی
  "avatar": { "height": 168, "bust": 90, … },  // اندازه‌های کاملِ بدن (سانتی‌متر)؛
                                     // مانکن با buildBody(avatar) ساخته می‌شود
  "fabric": {                        // شناسنامهٔ پارچه برای fabricLaw()
    "drape": 0.5,
    "physics": { "weight": …, "bending": …, "shear": …,
...
سه نکته برای موتورِ بیرونی:

- مرجعِ رفتاری، خودِ مرورگر است: `tests/js/bench-drape.mjs` همین بسته را
  می‌خواند و مو‌به‌مو همان خطِ لوله را اجرا می‌کند (چیدن، دوختِ بی‌وزنی،
  برگرداندن روی شانه، وزن، جوش، پس‌زدن). موتورِ تازه باید از همان سنجه‌ها
  رد شود، نه فقط «قشنگ به‌نظر برسد».
- مانکن را از `avatar` بسازید، نه از مدلِ خودتان — لباس به اندازه‌های همین
  مشتری بریده شده.
- نتیجهٔ حالتِ کیفیت باید با همان دستگاهِ بسته برگردد (سانتی‌متر، y رو به
  پایین) تا نماگرِ موجود بتواند نشانش بدهد.
