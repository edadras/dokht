...
<x-app-layout title="گزارش تست تناسب" wide>
    ...
    <div class="grid gap-5 lg:grid-cols-3">
        ...
        <div class="lg:col-span-2">
            {{--
                رندرِ سرور (Blender) و نمای زندهٔ مرورگر، هر دو.

                رندرِ سرور چند دقیقه طول می‌کشد و روی نصبی که کانتینرِ renderer
                ندارد هرگز نمی‌رسد؛ پس نمای مرورگر همیشه هست و تصاویرِ سرور —
                وقتی آماده شدند — بالای آن می‌نشینند. تا آن وقت، وضعیت هر چند
                ثانیه از مسیرِ render-status پرسیده می‌شود، نه با reload کردنِ
                کلِ صفحه.
            --}}
            @php
                $renderLabels = ['front' => 'جلو', 'side' => 'پهلو', 'back' => 'پشت', 'water' => 'افت پارچه سنگین', 'airflow' => 'اثر جریان هوا'];
                $renderStatus = $serverRender['status'] ?? 'none';
            @endphp
            <div x-data="{
                    ...
                    async poll() {
                        ...
                        try {
                            const response = await fetch(@js(route('simulations.render-status', $simulation)), { headers: { Accept: 'application/json' } });
                        ...
                        if (this.status === 'pending') { setTimeout(() => this.poll(), 8000); }
                    },
                        ...
                        <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                            <template x-for="[key, url] in Object.entries(images)" :key="key">
                                <figure class="overflow-hidden rounded-2xl border border-stone-200 bg-stone-950 shadow-sm">
                                    <img :src="url" :alt="labels[key] || key" loading="lazy" class="aspect-4/5 h-auto w-full object-cover">
                                    <figcaption class="border-t border-stone-800 px-3 py-2 text-center text-xs font-semibold text-stone-200" x-text="labels[key] || key"></figcaption>
                                </figure>
                            </template>
                        </div>
                        <div class="mt-3 flex flex-wrap items-center gap-3">
                            <template x-if="model">
                                <a :href="model" class="inline-flex items-center gap-2 rounded-xl border border-stone-300 bg-white px-4 py-2 text-sm font-semibold text-stone-800 hover:bg-stone-50">
                                    <x-icon name="download" class="h-4 w-4" />
                                    دریافت مدل سه‌بعدی GLB
                                </a>
                            </template>
                            <p class="text-xs leading-6 text-stone-500">
                                این تصاویر و مدل با موتور Blender روی سرور و روی همان تنِ دوخته‌شده ساخته شده‌اند.
                            </p>
                        </div>
                    </div>
                </template>
                <template x-if="status === 'pending'">
                    <div class="flex items-center gap-3 rounded-2xl border border-brand-200 bg-brand-50 px-4 py-3">
                        <x-icon name="refresh" class="h-5 w-5 shrink-0 animate-spin text-brand-600" />
                        <p class="text-sm text-brand-900">
                            رندرِ واقعی روی سرور در حال ساخت است؛ وقتی آماده شد همین‌جا نشان داده می‌شود. تا آن وقت نمای زندهٔ پایین را ببینید.
                        </p>
                    </div>
                </template>
                <template x-if="status === 'failed'">
                    <div class="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
                        موتورِ سرور نتوانست این رندر را کامل کند؛ گزارشِ خطا ثبت شده است. نمای زندهٔ پایین سرِ جایش است.
                    </div>
                </template>
            </div>
